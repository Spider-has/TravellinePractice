...
public class BookingService : IBookingService
{
    ...
    [
        new RoomCategory ( "Standard",  100,  10 ),
        new RoomCategory ( "Deluxe",  200,  5 )
    ];
    // Вызов конструкторов
    ...
    public Booking Book( int userId, string categoryName, DateTime startDate, DateTime endDate, Currency currency )
    {
        ...
        //Создаю booking через конструктор
        Booking booking = new( userId, startDate, endDate, selectedCategory, currency, totalCost );
    ...
    public void CancelBooking( Guid bookingId )
    {
        ...
        // решил добавить проверку (на всякий случай, теоретически функция может вернуть null)
        if ( category is null )
        {
            throw new ArgumentException( "Cannot find such category name" );
        }
    ...
    public IEnumerable<Booking> SearchBookings( DateTime startDate, DateTime endDate, string categoryName )
    {
        ...
        // В описании задачи сказано, что при поиске по фильтру дата учитывается включительно, значит <=, а не <
    ...
    public decimal CalculateCancellationPenaltyAmount( Booking booking )
    {
        ...
        // Вычисляю теперь не полную дату со временем, а только Date часть, иначе Datetime высчитывал разницу в часах до следующего дня и потом при вызове .Days возвращал 0, если указана завтрашняя дата
        // В текстовке задания сказано , что штраф - положительное число, значит нам нужно вычитать из startDate dateNow
        int daysBeforeArrival = ( booking.StartDate.Date - DateTime.Now.Date ).Days;
        return 5000.0m / daysBeforeArrival;
    }

    private static decimal GetCurrencyRate( Currency currency )
    {
        // Вот тут может быть ошибся, но ведь и доллар и юань дороже рубля, значит нужно currencyRate делить, а не умножать. ( Потому что у нас в структуре данные храняться как cost - currency ). 
        // С умножением мы получаем цену вместо (условно) 1 доллара за 1 день, 100 долларов за 1 день. Что, явно неправильно
        // Или мы работаем с условностью того, что рубль стал сверхвалютой дороже всех остальных)
    ...
    private static decimal CalculateBookingCost( decimal baseRate, int days, int userId, decimal currencyRate )
    {
        ...
        // тут надо умножать итоговую разницу на currencyRate, иначе вычисляется неправильно и получается отрицательное число
